 ...
 *
 * Structures defining the functions to build and decode macro/microinstructions with DLBs of small size.
 *
 */
...
uint64_t build_macroinstr (uint itt_idx, uint imm, OPC_STORAGE dst, OPC_STORAGE src, SWSIZE sw_len, uint shift_sa, uint rf_n, uint src0_n,
    ...
    if (sw_len != SWSIZE::INV) { // SA
        ...
        macroinstr <<= MACRO_SHIFTSA_BITS;
        macroinstr |= (shift_sa & ((1UL << MACRO_SHIFTSA_BITS) - 1));
...
SWSIZE get_ms_size (uint64_t macroinstr) {
    SWSIZE size;
    macroinstr >>= MACRO_RF_N_BITS+MACRO_DST_N_SA_BITS+MACRO_SHIFTSA_BITS+MACRO_SRC0_N_BITS+MACRO_DST_BITS+MACRO_SRC_BITS;
...
uint get_ms_dst_n (uint64_t macroinstr) {
    uint dst_n;
    macroinstr >>= MACRO_SRC_BITS + MACRO_DST_BITS + MACRO_SRC0_N_BITS + MACRO_SHIFTSA_BITS;
...
uint get_ms_csd_addr (uint64_t macroinstr) {
    uint src;
    src = (macroinstr >> (MACRO_DST_N_SA_BITS+MACRO_SHIFTSA_BITS+MACRO_SRC0_N_BITS+MACRO_DST_BITS+MACRO_SRC_BITS))\
...
void unpack_fields (uint64_t macrofields, bool mov_valid, bool sa_valid, bool pm_valid,
    ...
    *pm_fields = 0;
    ...
    if (sa_valid) {
        ...
        sa_shift = macro_aux & ((1UL << MACRO_SHIFTSA_BITS) - 1);
        macro_aux >>= MACRO_SHIFTSA_BITS;
    ...
    if (sa_valid) {
        ...
        *sa_fields <<= SA_SHIFT_BITS;
        *sa_fields |= uint(sa_shift) & ((1UL << SA_SHIFT_BITS) - 1);
...
uint16_t build_sa_microinstr (ADDOP sa_add) {
    uint16_t microinstr = 0;
...
void decode_sa_microinstr (uint16_t microinstr, uint32_t fields, ADDOP* sa_add, uint* sa_shift,
    ...
    *dst = sa_dst_to_opc(dst_temp);
    fields >>= SA_DST_BITS;
    *sa_shift = (fields & ((1UL << SA_SHIFT_BITS) - 1));
    fields >>= SA_SHIFT_BITS;
